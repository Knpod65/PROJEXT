...
Supporting finance roster XLSX is implemented and validated with five sheets, live `Supervision`, authoritative `PaperDistributionAssignment`, one-person-per-slot deduplication, and top-two physical-room mapping. Safety flags remain `payment_authorization_enabled=false`, `final_export_enabled=false`, and `DRAFT_NOT_AUTHORIZED`. Final authorization remains `FINAL_AUTHORIZATION_DESIGN_BLOCKED`; production readiness is unchanged.
