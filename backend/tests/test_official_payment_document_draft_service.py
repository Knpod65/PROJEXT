...
def test_draft_uses_confirmed_120_200_rates_and_ignores_supervision_compensation():
    ...
    assert result["metadata"]["rate_source"] == "OFFICIAL_2_2568_SAMPLE_RATE_CONFIRMED_BY_USER"
...
def test_blocked_ineligible_invigilation_rows_are_excluded_with_warning():
    ...
    result = build_official_payment_document_draft_from_sources([schedule], [_period()], _payload())
    assert result["rows"] == []
